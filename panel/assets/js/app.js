...
!function ($) {
    ...
    $(document).on('click', '.find', function(e){
        ...
        var Tombol = "<button type='button' class='btn btn-secondary waves-effect waves-light' data-dismiss='modal'>Close</button>";

        $('#PopupContent').load($(this).attr('href'));

        $('#PopupGue').modal('show');
        $('#PopupFooter').html(Tombol);
    });

    $('#PopupGue').on('hidden.bs.modal', function(){
        $('#PopupContent').html('');
        $('#PopupHeader').html('');
        $('#PopupFooter').html('');
    })
